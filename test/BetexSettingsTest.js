  ...
  describe("GIVEN un usuario desea configurar betex con Gondwana", async () => {
    describe("AND consulta los valores por default", async () => {
      ...
      it("THEN maxAmountWeiPerDay del usuario deben ser los mismos que por default", async () => {
        assert(
          defaultSettings[0].eq(defaultValues.defaultMaxAmountWeiPerDay),
          "maxAmountWeiPerDay diferente a default"
        );
      });
      it("THEN maxAmountBtxPerDay del usuario deben ser los mismos que por default", async () => {
        assert(
          defaultSettings[1].eq(defaultValues.defaultMaxAmountBtxPerDay),
          "maxAmountBtxPerDay diferente a default"
        );
      });
      it("THEN maxAmountWeiPerDay del usuario deben ser los mismos que por default", async () => {
        assert(
          defaultSettings[2].eq(defaultValues.defaultMaxBetsPerDay),
          "maxBetsPerDay diferente a default"
        );
      });
    });
    describe("AND configura nuevos valores", async () => {
      ...
      beforeEach(async () => {
        await betexMobileGondwana.saveUserSettings(
          maxAmoutWei,
          maxAmountBtx,
          maxCantApuestas
        );
      ...
      it("THEN maxAmountWeiPerDay del usuario deben ser los mismos que a los de configuración", async () => {
        assert(
          settings[0].eq(maxAmoutWei),
          "maxAmountWeiPerDay diferente a configurado"
        );
      });
      it("THEN maxAmountBtxPerDay del usuario deben ser los mismos que  a los de configuración", async () => {
        assert(
          settings[1].eq(maxAmountBtx),
          "maxAmountBtxPerDay diferente a configurado"
        );
      });
      it("THEN maxAmountWeiPerDay del usuario deben ser los mismos que   a los de configuración", async () => {
        assert(
          settings[2].eq(maxCantApuestas),
          "maxBetsPerDay diferente a configurado"
        );
      });
    });
  });
